...
class Model():
    ...
    def run(self, doc_pt, res_dir):
        ...
        # 初始化 self.nb_z 和 self.nwz  #self.nb_z: 表示的是在那么多的词对中，每个主题出现的次数。#self.nwz[2][3] 表示的是在出题2中，2号单词出现的次数。
        self.model_init()
        # print('============================')
        # print(self.nwz)
        ...
        for i in range(1, self.n_iter+1):
            ...
            for b in range(len(self.bs)):
                # 根据每个biterm更新文章中的参数。
                # 计算核心代码，self.bs中保存的是词对的biterm类，代码是对每一个词对进行更新的。
    ...
    def reset_biterm_topic(self, bi):
        ...
        self.nwz[k][w1] -= 1
        self.nwz[k][w2] -= 1
        assert (self.nb_z[k] > -10e-7 and self.nwz[k][w1]
                > -10e-7 and self.nwz[k][w2] > -10e-7)
        bi.reset_z()
    ...
    def comput_pz_b(self, bi, pz):
        ...
        for k in range(self.K):
            if (self.has_background and k == 0):
                ...
                pw1k = (self.nwz[k][w1] + self.beta) / \
                    (2 * self.nb_z[k] + self.W * self.beta)
                pw2k = (self.nwz[k][w2] + self.beta) / \
                    (2 * self.nb_z[k] + 1 + self.W * self.beta)

            pk = (self.nb_z[k] + self.alpha) / (len(self.bs) +
                                                self.K * self.alpha)  # len(self.bs)表示的是在文档中以后多少的词对
            pz[k] = pk * pw1k * pw2k
    ...
    def save_pw_z(self, pt):
        ...
        for k in range(self.K):
            for w in range(self.W):
                pw_z[k][w] = (self.nwz[k][w] + self.beta) / (self.nb_z[k]
                                                             * 2 + self.W * self.beta)  # 计算每个词在这个主题中出现的概率。
